...
func BenchmarkAssembleBlock(b *testing.B) {
	...
	ledgerName := fmt.Sprintf("%s-mem-%d-%d", b.Name(), b.N, crypto.RandUint64())
	const inMem = true
...
func BenchmarkAssembleBlockSteadyState(b *testing.B) {
	b.StopTimer()
	const numUsers = 100
	const txPoolSize = 6000
	log := logging.TestingLog(b)
	...
	for i := 0; i < numUsers; i++ {
		...
		genesis[addr] = basics.AccountData{
			Status:     basics.Online,
			MicroAlgos: basics.MicroAlgos{Raw: 1000000000000000},
		}
	}
	...
	require.Equal(b, len(genesis), numUsers+1)
	genBal := bookkeeping.MakeGenesisBalances(genesis, sinkAddr, poolAddr)
	ledgerName := fmt.Sprintf("%s-steady-mem-%d-%d", b.Name(), b.N, crypto.RandUint64())
	const inMem = true
	cfg := config.GetDefaultLocal()
	cfg.Archival = true
	ledger, err := data.LoadLedger(log, ledgerName, inMem, protocol.ConsensusCurrentVersion, genBal, genesisID, genesisHash, cfg)
	...
	txPoolConfig := config.GetDefaultLocal()
	txPoolConfig.TxPoolSize = txPoolSize
	txPoolConfig.EnableAssembleStats = false
	txPool := pools.MakeTransactionPool(ledger.Ledger, txPoolConfig, logging.Base(), nil)
	next := ledger.NextRound()
	sourceIndex := 0

	fillPool := func() {
		for txPool.PendingCount() < txPoolSize {
			destIndex := rand.Intn(len(addresses))
			for destIndex == sourceIndex {
				destIndex = rand.Intn(len(addresses))
			}
			...
			tx := transactions.Transaction{
				...
				PaymentTxnFields: transactions.PaymentTxnFields{
					Receiver: addresses[destIndex],
					Amount:   basics.MicroAlgos{Raw: mockBalancesMinBalance + (rand.Uint64() % 10000)},
				},
			}
			crypto.RandBytes(tx.Note)

			signedTx := tx.Sign(secrets[sourceIndex])
			require.NoError(b, txPool.Remember([]transactions.SignedTxn{signedTx}))
			sourceIndex = (sourceIndex + 1) % len(addresses)
		}
	}

	fillPool()
	deadline := time.Now().Add(time.Second)
	block, err := txPool.AssembleBlock(next, deadline)
	require.NoError(b, err)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		require.NoError(b, ledger.AddBlock(block.UnfinishedBlock(), agreement.Certificate{Round: next}))

		b.StartTimer()
		txPool.OnNewBlock(block.UnfinishedBlock(), ledgercore.StateDelta{})
		b.StopTimer()
